...
function Link($target, $path, $kind) {   # kind: dir (junction) or file (symlink, else copy)
  ...
  if ($item -and ($item.Attributes -band [IO.FileAttributes]::ReparsePoint)) {
    ...
    if (-not $current.StartsWith("$repo\")) { "  SKIP   $path links outside this repo ($current)"; return }
    "  relink $path"
...
foreach ($h in $harnesses) {
  foreach ($link in Get-ChildItem "$HOME\.$h\skills" -Force -ErrorAction SilentlyContinue | Where-Object { $_.Attributes -band [IO.FileAttributes]::ReparsePoint }) {
    $t = $link.Target -join ''
    if ($t.StartsWith("$repo\") -and -not (Test-Path "$t\SKILL.md")) { "  remove $($link.FullName) (skill gone)"; if (-not $DryRun) { [IO.Directory]::Delete($link.FullName) } }
  }
}
